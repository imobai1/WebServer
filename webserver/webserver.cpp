...
void webserver::thread_pool() {
    //线程池
    m_pool = new threadpool<http_connection>(m_actormodel, m_connPool, m_thread_num);
}
...
void webserver::log_write() {
    if (0 == m_close_log)
    {
        //初始化日志
        if (1 == m_log_write)
            m_log::get_instance()->init("./ServerLog", m_close_log, 2000, 800000, 800);
        else
            m_log::get_instance()->init("./ServerLog", m_close_log, 2000, 800000, 0);
    }
}
