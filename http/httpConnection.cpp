...
void http_connection::init(int sockfd, const sockaddr_in &addr, char* root, int TRIGMode , int close_log,
...
sockaddr_in *http_connection::get_address() {
    return &m_address;
}

void http_connection::initMysql_result(SqlConnectPool *connPool) {
    //从连接池中去一个连接
    MYSQL *mysql = nullptr;
...
bool http_connection::process_write(http_connection::HTTP_CODE ret) {
    switch (ret){
        case INTERNAL_ERROR:{
